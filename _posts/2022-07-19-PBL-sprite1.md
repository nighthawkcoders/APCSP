...
<script>
  ////////// convert liquid hash to javascript key value objects /////////
  ...
  window.addEventListener("keydown", (event) => {
    if (event.key === "ArrowRight") {
      ...
      if (event.repeat) { //on hold key
        stopAnimate();
        startAnimate(obj["Rest"],0);  //rest animation 
      } else { //on tap key
      ...
      if (event.repeat) { //on hold key
        ...
        stopAnimate();
        startAnimate(obj["Rest"],0); //resting animation
      }
    }
  });
