...
```
> Anaconda Install.  
- Download for MacOS: [https://www.anaconda.com/products/distribution](https://www.anaconda.com/products/distribution)
...
> WSL Command / Powershell Terminal updates
```bash
...
```
> Anaconda Install.  
- Install as indicated, if it fails obtain latest Linux-x86 distribution link at the bottom of page: https://www.anaconda.com/products/distribution.
```bash
cd ~
...
> Install JavaScript kernel.  The outputs from these are verbose so everything is abbreviated to commands.
```bash
